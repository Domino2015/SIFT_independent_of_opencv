...
*          利用图像间的差分代替微分。 
**************************************************************************************************************************/  
void DogPyramid(const std::vector<Mat>& gauss_pyr, std::vector<Mat>& dog_pyr, int octaves, int intervals=INTERVALS)  
{  
    for(int o = 0; o < octaves; o ++)  
    {  
        for(int i = 1; i < intervals+3; i++)  
...
double PyrAt(const std::vector<Mat>& pyr, int index, int x, int y)  
{  
...
//3维D(x)一阶偏导,dx列向量  
void DerivativeOf3D(int x, int y, const std::vector<Mat>& dog_pyr, int index, double *dx)  
{  
...
void Hessian3D(int x, int y, const std::vector<Mat>& dog_pyr, int index, double *H)  
{  
...
**************************************************************************************************************************/  
//计算x^  
void GetOffsetX(int x, int y, const std::vector<Mat>& dog_pyr, int index, double *offset_x)  
{  
...
//计算|D(x^)|  
double GetFabsDx(int x, int y, const std::vector<Mat>& dog_pyr, int index, const double* offset_x)  
{  
...
**************************************************************************************************************************/  
Keypoint* InterploationExtremum(int x, int y, const std::vector<Mat>& dog_pyr, int index, int octave, int interval, double dxthreshold = DXTHRESHOLD)  
{  
    //计算x=(x,y,sigma)^T  
    //x^ = -H^(-1) * dx; dx = (Dx, Dy, Ds)^T  
    double offset_x[3]={0};  
...
**************************************************************************************************************************/  
void DetectionLocalExtrema(const std::vector<Mat>& dog_pyr, std::vector<Keypoint>& extrema, int octaves, int intervals=INTERVALS)  
{  
...
*      
**************************************************************************************************************************/  
void CalculateScale(std::vector<Keypoint>& features, double sigma = SIGMA, int intervals = INTERVALS)  
{  
    double intvl = 0;  
...
void CalcOriFeatures(const Keypoint& keypoint, std::vector<Keypoint>& features, const double *hist, int n, double mag_thr)  
{  
    double  bin;  
...
*        8）至此，SIFT特征描述向量生成。 
*********************************************************************************************************************************/  
void DescriptorRepresentation(std::vector<Keypoint>& features, const std::vector<Mat>& gauss_pyr, int bins, int width)  
{  
    double ***hist;  
...
*        2---std::vector<Keypoint>& features---用来存储检测出来的关键点 
